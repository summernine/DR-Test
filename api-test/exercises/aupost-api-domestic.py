...
def test_get_postage_domestic_services_status_code_within_AU(fr, to):
    ...
    response = requests.get(f"{domestic_url}/service.json?from_postcode={fr}&to_postcode={to}&length=22&width=16&height=12&weight=1.5",
                            headers = auth)
...
def test_get_postage_domestic_cost_within_AU(fr, to, service_code, service_name):
    domestic_url = "https://digitalapi.auspost.com.au/postage/parcel/domestic"
    auth = {"AUTH-KEY": "60c2ab35-bff5-4082-9184-a6f8ec713a81"}
    response = requests.get(f"{domestic_url}/calculate.json?from_postcode={fr}&to_postcode={to}&length=22&width=16&height=12&weight=1.5&service_code={service_code}",
                            headers= auth)
    print(response.json())
